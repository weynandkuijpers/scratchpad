### Parent license acquisition (purchase).

```mermaid
graph LR

subgraph User Device
    user(User) --> |Installs Skipr App on User device| private_key
    private_key(Private Key) -->|1. P2P standard Algo| public_key_1(Public Key 1)
    private_key -->|2. Simple Hash Algo| public_key_2(Public Key 2)
end

subgraph Skipr backend
    user -->|Purchases trial license| skipr_agent(Skipr Agent)
    public_key_2 -->|User device sends Public Key 2| skipr_agent
    skipr_agent --> |Completes Trial / Purchased lincense sales| skipr_agent
    skipr_agent --> | `Public Key 2` | skipr_chain(Skipr Chain)
end
```

### Child license acquisition (borrow).

```mermaid
graph LR

subgraph Parent Device
    user_1(User 1) --> p_private_key(Parent Private Key)
    p_private_key --> p_public_key_1(Parent Public key 1)
    p_private_key --> p_public_key_2(Parent Public key 2)

end

subgraph Child Device
    user_2(User 2) --> |1. Installs Skipr App on child device| c_private_key(App creates private key)
    c_private_key -->|2. P2P Hash Algo| c_public_key_1(Child Public Key 1)
    c_private_key -->|3. Simple Hash Algo| c_public_key_2(Child Public Key 2)
end

subgraph Offline Key exchange
    c_public_key_2 --> | Public Key 2 as QR code| user_2_screen(Child Device Screen)
    user_2_screen --> |Read QR Code| user_1
end

subgraph Skipr Backend
    user_1 --> |Send Child Public Key 2| agent_1
    agent_1 --> |Enter Child Public Key 2 as license| skipr_chain(Skiprs Chain)
end
...
- Bob has purchased a Skipr (parent) License and now has a valid license + app installed
- Alice meets Bob and wants to use one of Bob's child licenses.
- Alice installs the app
- The app asks Alice if she wants to purchase a license or use a licensed persons child's license slot.  Alice opts to use Bob's child's license
- The app on Alice's phone presents a QR code (of the secondary public key)
- Bob selects the "share license" button on the app
- Bob sees the current number of people who he is sharing licenses with.  There is a free slot available for Alice's
- Bob selects the free slot and enters Alice's (pseudo)name
- Bob's app asks to scan Alice's QR code
- Bob's app get Alice's public key (2) and instructs the agent to create a child license entry in Skipr's chain.
- Alice can now independently use the Skipr License.  
- Alice cannot allow other people to obtain a Skipr license.  For this she has to purchase a license herself
...
sequenceDiagram
    ...
    participant Bob App
    participant Agent
    participant Skipr Chain

    Alice->>Bob: Would like to have access to Skipr's Ultimate VPN service
    Bob->>Alice: That's fine, I have a packaged deal with child licenses.  Please download the Skipr App
    Alice->>Alice: Downloading Skipr App
    Bob->>Alice: Now say that you want to use other child license services
    Alice->>Alice App: Click "Other persons child license"
    Alice App->> Alice: QR code for license holder (QR code = Alice's secondary public key)
    Alice->>Bob: QR code for license holder
    Bob->>Bob App: Scan QR code for of Alice App
    Bob App->>Alice App: Scan QR code
    Bob App->>Bob App: Get all current package license holders, one slot free?
    Bob App->>Bob: Enter name for new package license holder
    Bob->>Bob App: Enter Alice pseudo name
    Bob App->>Agent: Store new child license in chain
    Agent->>Bob App: okay (confirm action)
    Agent->>Skipr Chain: Store new package license in chain
    Alice->>Agent: Deploy VPN, I should have license for my <secondary public_key>
    Agent->>Alice: license verfied, deploying ultimate Skipr VPN.
